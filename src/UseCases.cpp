...
bool UseCases::checkConectividade2Points(Graph<VerticeInfo> graph, Vertex<VerticeInfo> vInicial,
                                         Vertex<VerticeInfo> vFinal) {
...
vector<vector<VerticeInfo>>
UseCases::determinarRotaCamioes(PontoPartida pontoPartida, CentroReciclagem centroReciclagem,
    ...
    vector<vector<VerticeInfo>> verticesOrdered = minimizarDistPercorrida(graph, camioesNecessarios, pontosParaRecolher,
                                                                          pontoPartida, centroReciclagem);
...
UseCases::minimizarDistPercorrida(Graph<VerticeInfo> &graph, vector<Camiao> camioesNecessarios,
                                  vector<PontoRecolha> pontosRecolha, PontoPartida pontoPartida,
                                  CentroReciclagem centroReciclagem) {
...
void
UseCases::obterPontosRecolhaMaisProximo(vector<PontoRecolha> pontosRecolha, VerticeInfo oldPonto, int tipoLixoAtual,
                                        Graph<VerticeInfo> graph, vector<VerticeInfo> &listToReturn) {
    ...
    for (int j = 0; j < pontosRecolha.size(); j++) {
        for (int k = 0; k < pontosRecolha.at(j).getTipoLixo().size(); k++) {
            if (pontosRecolha.at(j).getTipoLixo().at(k) ==
                tipoLixoAtual) { //apenas para os pontos de recolha do tipoLixoAtual
...
vector<VerticeInfo>
UseCases::getEdgesFromVertexes(PontoPartida pontoPartida, vector<VerticeInfo> pontosRecolha, Graph<VerticeInfo> graph) {
...
vector<VerticeInfo>
UseCases::determinarRotaUE(PontoPartida pontoPartida, CentroReciclagem centroReciclagem, double capMax,
                           Graph<VerticeInfo> graph) {

    //1º Detetar todos os contentores domesticos que estao acima da taxa viável
    vector<PontoRecolhaDomiciliario> pontosParaRecolher = getPontosDomesticosAcimaTaxaViavel(graph);

    //2º Obter a rota com a menor distancia
    vector<PontoRecolhaDomiciliario> listToReturn;
    obterPontosRecolhaDomesticosMaisProximo(pontosParaRecolher,pontoPartida,graph, listToReturn);


    //3º Atendendo a capMax do UE limitar a sua rota
    vector<VerticeInfo> pontosParaVisitar = determinarPontosDependendoDaCapMax(listToReturn,capMax);
    pontosParaVisitar.push_back(centroReciclagem);   //ultima iteracao para o centro de recolha

    //4º Preencher os pontos intermedios para o UE saber o caminho
    vector<VerticeInfo> caminho = getEdgesFromVertexes(pontoPartida,pontosParaVisitar,graph);
...
vector<PontoRecolhaDomiciliario> UseCases::getPontosDomesticosAcimaTaxaViavel(Graph<VerticeInfo> graph) {
    vector<PontoRecolhaDomiciliario> pontosParaRecolher; //pontos a recolher
    vector<Vertex<VerticeInfo> *> vertices = graph.getVertexSet();
    ...
    for (int i = 0; i < vertices.size(); i++) {
        ...
        if (instanceof<PontoRecolhaDomiciliario>(info)) {
            auto *pontoRecolha = dynamic_cast<PontoRecolhaDomiciliario *>(info);
            vector<double> taxasOcupacao = pontoRecolha->getTaxasOcupacao();
            for (int j = 0; j < taxasOcupacao.size(); j++) {
                double taxa = taxasOcupacao.at(j);
                if (taxa > taxaViavel)
...
void
UseCases::obterPontosRecolhaDomesticosMaisProximo(vector<PontoRecolhaDomiciliario> pontosRecolha, VerticeInfo oldPonto,
                                                  Graph<VerticeInfo> graph, vector<PontoRecolhaDomiciliario> &listToReturn) {
    Vertex<VerticeInfo> *debug = graph.findVertex(&oldPonto);
    VerticeInfo verticeInfoDebug = *(debug->getInfo());

    double minDist = DBL_MAX;
    Vertex<PontoRecolhaDomiciliario> *bestVertex = nullptr;
    graph.dijkstraShortestPath(oldPonto);
    for (int j = 0; j < pontosRecolha.size(); j++) {

        //apenas para os pontos de recolha do tipoLixoAtual
...
/*                for(VerticeInfo verticeInfo : shortestPath){
        ...
        Vertex<VerticeInfo> *vertexTemp = graph.findVertex(&pontosRecolha.at(j));
        VerticeInfo *verticeInfoTemp = vertexTemp->getInfo();
        auto *pontoRecolhaAtual = dynamic_cast<PontoRecolhaDomiciliario *>(verticeInfoTemp);
        Vertex<PontoRecolhaDomiciliario> *vertex = new Vertex<PontoRecolhaDomiciliario>(pontoRecolhaAtual);
        bool different = true;
        for (VerticeInfo verticeInfo : listToReturn) {
            if (verticeInfo == *(vertex->getInfo()))
                different = false;
        ...
        if (different) { //encontrou o lixo mais perto e ainda nao foi adicionado a lista a retornar
            ...
            if (dist < minDist) {
                ...
                bestVertex = new Vertex<PontoRecolhaDomiciliario>(*vertex);
            }
        }
    ...
    oldPonto = *(bestVertex->getInfo());
    if (pontosRecolha.size() > listToReturn.size())
        obterPontosRecolhaDomesticosMaisProximo(pontosRecolha, oldPonto, graph, listToReturn);
}

vector<VerticeInfo> UseCases::determinarPontosDependendoDaCapMax(vector<PontoRecolhaDomiciliario> pontosRecolha, double capMax) {

    vector<VerticeInfo> pontos;
    double capAtual = 0;
    for(PontoRecolhaDomiciliario pontoRecolha : pontosRecolha){

             for(int i = 0; i < pontoRecolha.getTaxasOcupacao().size(); i++){
                 if(capAtual + pontoRecolha.getTaxasOcupacao().at(i) * pontoRecolha.getCapacidadesMax().at(i) < capMax){
                     capAtual += pontoRecolha.getTaxasOcupacao().at(i) * pontoRecolha.getCapacidadesMax().at(i);
                     pontos.push_back(pontoRecolha);
                 }
